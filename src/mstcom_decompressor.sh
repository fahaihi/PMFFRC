...
#author: SH
#date: 2022/10/15
#describe: A script for P-MFFRC decompressor using mstcom algorithm
#######################################################################################
de_path=$1
Pr=$2
clean_debug=$3
...
mstcom_decompressor() {
  echo "  call mstcom algorithm for de-compression!"
  files_list=$(ls)
  for tempFile in ${files_list}; do
    if [[ ${tempFile:$((${#tempFile} - 7))} == ".mstcom" ]]; then
  ...
  echo "  call mstcom algorithm for de-compression over!"
}
mstcom_decompressor
if [ $? -ne 0 ]; then
