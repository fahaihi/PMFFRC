...
if __name__ == '__main__':
    ...
    command = "cut -f1 -d' ' " + output + "/sort_seq > " + input.split(sep=".")[0] + ".sorted_seq1"
    subprocess.check_call(command, shell=True)
    ...
    start_scoring_time = timeit.default_timer()
    command = "zpaq -method 5 a " + input.split(sep=".")[0] + ".zq_seq " + input.split(sep=".")[0] + ".sorted_seq1 " + \
              input.split(sep=".")[0] + ".sorted_seq_id -threads" + threads
    subprocess.check_call(command, shell=True)
    end_scoring_time = timeit.default_timer()
    ...
    size1 = os.path.getsize(input.split(sep=".")[0] + ".sorted_seq1") + os.path.getsize(input.split(sep=".")[0] + ".sorted_seq_id")
    print("zq_seq:%d " % size)
    print("source_seq:%d " % size1)
